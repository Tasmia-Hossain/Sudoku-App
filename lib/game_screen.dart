 ...
 */
  ...
  Widget _buildCell(int row, int col) {
    ...
    final thickBorder = BorderSide(width: 2.0, color: Colors.black);
    final thinBorder = BorderSide(width: 1.0, color: Colors.black);
      ...
      child: Container(
        ...
        decoration: BoxDecoration(
          border: Border(
            ...
            bottom: (row == 2 || row == 5 || row == 8) ? thickBorder : thinBorder,
            left: col == 0 ? thickBorder : thinBorder,
            right: (col == 2 || col == 5 || col == 8) ? thickBorder : thinBorder,
          ),
          color: (selectedCell[0] == row || selectedCell[1] == col)
              ? Colors.purple.shade50 // Highlight selected cell and row/column
              : (isBold ? Colors.grey.shade400 : null),
        ),
        child: Text(
          sudokuBoard[row][col] != 0 ? sudokuBoard[row][col].toString() : '',
          style: TextStyle(
    ...
    return WillPopScope(
      onWillPop: () async {
        ...
        Navigator.pushReplacement(
          context,
          MaterialPageRoute(builder: (context) => Loading()), //HomePage
        );
